...
def get_cifar(n_cls=10, image_size=32, test=False):
    if n_cls == 10:
        return get_cifar10(image_size, test=test)
    elif n_cls == 100:
        return get_cifar100(image_size, test=test)
    else:
        raise ValueError("The number of class for CIFAR must be 2 to 100.")
        print("But, {} was given.".format(n_cls))


def get_cifar10(image_size=32, test=False):
...
def get_cifar100(image_size=32, test=False):
